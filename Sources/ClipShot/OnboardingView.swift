...
struct OnboardingView: View {
    ...
    private var folderPage: some View {
        VStack(spacing: 18) {
            ...
            Text("Choose a Screenshots Folder")
                .font(.title2.bold())
            Text("This is the only folder ClipShot will ever look at. Desktop, Documents, and Downloads can't be chosen — macOS blocks background apps from watching them reliably.")
                .multilineTextAlignment(.center)
                .foregroundStyle(.secondary)
                .frame(maxWidth: 400)
    ...
    private var finishPage: some View {
        VStack(spacing: 18) {
            ...
            Text("All Set")
                .font(.title.bold())
            Text("ClipShot will now watch **\((chosenFolder ?? AppState.defaultSuggestedFolder).path)** and copy every new screenshot straight to your clipboard. It'll keep running in the background even after you close this window or restart your Mac.")
                .multilineTextAlignment(.center)
                .foregroundStyle(.secondary)
                .frame(maxWidth: 420)
            Spacer()
