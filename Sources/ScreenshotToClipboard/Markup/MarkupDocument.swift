...
/// committed elements plus undo/redo history. Coordinates are in the base
/// image's native pixel space (see DrawingElement) so they stay correct
/// no matter how the window is resized or zoomed mid-edit.
final class MarkupDocument: ObservableObject {
    ...
    func erase(at point: CGPoint) {
        ...
        let erasedID = elements[idx].id
        snapshot()
        elements.remove(at: idx)
        if selectedID == erasedID { selectedID = nil }
    }

    /// Applied when the color/stroke-width swatches are changed while an
    /// element is selected, so picking a new color acts on what you
    /// selected rather than only affecting elements drawn from now on.
    func restyleSelected(color: Color? = nil, lineWidth: CGFloat? = nil) {
        guard let id = selectedID, let idx = elements.firstIndex(where: { $0.id == id }) else { return }
        snapshot()
        if let color { elements[idx].color = color }
        if let lineWidth { elements[idx].lineWidth = lineWidth }
    }
